...
class Tomarket:
    ...
    def main(self):
        while True:
                ...
                try:
                    ...
                    if token:
                        ...
                        if check_in["status"] == 200:
                            self.log(f"{green}Günlük kontrol başarılı")
                        else:
                            self.log(f"{yellow}Zaten kontrol edildi")

                        if "farming" not in user_info.keys():
                            self.log(f"{yellow}Farming henüz başlatılmadı")
                            start_farming = self.start_farming(token=token)
                            if start_farming.status_code == 200:
                                self.log(f"{green}Farming başlatıldı")
                                user_info = self.balance(token=token).json().get("data")
                            else:
                                self.log(f"{red}Farming başlatma başarısız")
                        ...
                        if current_time > end_farming_time:
                            ...
                            if end_farming.status_code == 200:
                                self.log(f"{green}Farming sonlandırıldı")
                                user_info = self.balance(token=token).json().get("data")
                                balance = user_info["available_balance"]
                                self.log(f"{green}Güncel bakiye: {white}{balance}")
                                time.sleep(10)
                                start_farming = self.start_farming(token=token)
                                if start_farming.status_code == 200:
                                    self.log(f"{green}Farming başlatıldı")
                                ...
                                self.log(f"{red}Farming sonlandırma başarısız")
                        else:
                            self.log(f"{yellow}Henüz talep etme zamanı değil")

                        readable_time = datetime.fromtimestamp(
                            end_farming_time
                        ).strftime("%Y-%m-%d %H:%M:%S")
                        self.log(f"{green}Farming sonlanma zamanı: {white}{readable_time}")
                        ...
                        if available_tickets > 0:
                            self.log(
                                f"{green}Mevcut biletler: {white}{available_tickets}"
                            )
                            while True:
                                self.log(f"{yellow}Oyun oynamaya başla")
                                start_game = self.start_game(token=token)
                                if start_game.status_code == 200:
                                    self.log(f"{yellow}Oyun oynanıyor, 30s bekle...")
                                    time.sleep(30)
                                    ...
                                    if claim_game.status_code == 200:
                                        ...
                                        balance = user_info["available_balance"]
                                        self.log(
                                            f"{green}Güncel bakiye: {white}{balance}"
                                        )
                                        tickets_left = user_info["play_passes"]
                                        self.log(
                                            f"{green}Kalan biletler: {white}{tickets_left}"
                                        )
                                        if tickets_left == 0:
                                            break
                                    else:
                                        self.log(f"{red}Oyun puanı talep etme başarısız")
                                else:
                                    self.log(f"{red}Oyun başlatma başarısız")
                        else:
                            self.log(f"{yellow}Mevcut oyun bileti yok")
                    else:
                        self.log(f"{red}Token bulunamadı!!!")
                except Exception as e:
                    self.log(f"{red}Auth verilerini alma hatası!!!")
            ...
            wait_message_parts = []
            if wait_hours > 0:
                wait_message_parts.append(f"{wait_hours} saat")
            if wait_minutes > 0:
                wait_message_parts.append(f"{wait_minutes} dakika")
            if wait_seconds > 0:
                wait_message_parts.append(f"{wait_seconds} saniye")

            wait_message = ", ".join(wait_message_parts)
            self.log(f"{yellow}Bekle {wait_message}!")
